...
class CodingTimeSimulator {
  ...
  private drawReadiness(): number {
    ...
    return Math.max(
      0.5,
      Math.min(1, this.readinessMean + this.readinessStd * z)
    );
  }
  ...
  private simulateTime(): number {
    ...
    return (
      W *
      N *
      (1 + this.retryImpact * Math.pow(N, this.retryPower)) *
      (this.linesOfCode / (R * 60))
    );
  }
  ...
  async plotDistribution(times: number[], useLogScale: boolean): Promise<void> {
    ...
    const filteredTimes =
      this.maxMinutes > 0 ? times.filter((t) => t <= this.maxMinutes) : times;
    const maxTime =
      this.maxMinutes > 0 ? this.maxMinutes : Math.max(...filteredTimes);
    ...
    const spec: Spec = {
      ...
      background: "white",
      title: {
        text: `AI Code Generation Time Distribution (N=${times.length}) ${
          useLogScale ? "log scale" : "linear scale"
        }`,
        subtitle: [
          `Parameters: AI success rate (Normal): ${
            this.readinessMean * 100
          }% mean, AI retries (Log-normal): ${Math.exp(this.mu).toFixed(
            1
          )} mean, AI response time (Normal): ${this.waitTimeMean}s mean`,
          `Expected time to generate ${this.linesOfCode} LOC, min:`,
          `Time range: ${Math.round(stats.minMinutes)}-${Math.round(
            stats.maxMinutes
          )}`,
          `Mean: ${Math.round(stats.meanMinutes)}`,
          `Median: ${Math.round(stats.medianMinutes)}`,
          `P90-P95: ${Math.round(stats.p90Minutes)}-${Math.round(
            stats.p95Minutes
          )}`,
        ],
        fontSize: 16,
    ...
    const filename = `media/sim_r${this.readinessMean}_w${
      this.waitTimeMean
    }_mu${this.mu}_s${this.sigma}_i${this.retryImpact}_p${this.retryPower}_${
      this._useLogScale ? "log" : "linear"
    }.png`;
  ...
  async plotCumulativeDistribution(
    times: number[],
    useLogScale: boolean
  ): Promise<void> {
    const filteredTimes =
    ...
    const canvas = await view.toCanvas();
    const nodeCanvas = canvas as unknown as Canvas;
    const filename = `media/sim_cumulative_r${this.readinessMean}_w${
      this.waitTimeMean
    }_mu${this.mu}_s${this.sigma}_i${this.retryImpact}_p${this.retryPower}_${
      this._useLogScale ? "log" : "linear"
    }.png`;
...
+0.05 → AI alternates between good and bad completions more.\n`,
  ...
  Object.entries(parameterDescriptions).forEach(([param, desc]) => {
    ...
    process.stdout.write(
      `${param.padEnd(15)} = ${String(value).padEnd(6)} | ${desc}\n`
    );
  });
...
async function main(): Promise<void> {
  ...
  const times = baseSimulator.runSimulation(10000);
  printSimulationReport(baseSimulator, times);

  await baseSimulator.plotDistribution(times, baseSimulator.useLogScale);
  await baseSimulator.plotCumulativeDistribution(
    times,
    baseSimulator.useLogScale
  );
}
